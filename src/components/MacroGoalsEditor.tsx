...
const GOAL_OPTIONS = [
  { key: 'weight_loss', label: 'Weight Loss', emoji: '📉', description: 'Lose 1-2 lbs per week' },
  { key: 'maintenance', label: 'Maintain Weight', emoji: '⚖️', description: 'Maintain current weight' },
  { key: 'muscle_gain', label: 'Muscle Gain', emoji: '💪', description: 'Gain muscle and strength' },
] as const;
...
export const MacroGoalsEditor: React.FC<MacroGoalsEditorProps> = ({
  ...
  const [selectedGoal, setSelectedGoal] = useState<'weight_loss' | 'maintenance' | 'muscle_gain'>('maintenance');
  const [recommendations, setRecommendations] = useState<MacroRecommendations | null>(null);
  ...
  const handleSaveGoals = async () => {
    ...
    try {
      ...
      HapticService.error();
      Alert.alert("Error", "Could not save your macro goals. Please try again.");
    }
  };
  ...
  const calculateTDEE = () => {
    ...
    // Apply smart defaults based on selected goal
    const smartDefaults = TDEECalculator.getSmartDefaults(fullData, selectedGoal);
    setGoals({
  ...
  const renderTDEECalculator = () => (
    <Card variant="primary" marginBottom="md">
      <Text variant="h3" marginBottom="sm">🧮 TDEE Calculator</Text>
      <Text variant="body" color="secondaryText" marginBottom="md">
        Calculate your recommended calories based on your stats and activity level
      </Text>
      
      {/* Goal Selection */}
      <Text variant="body" fontWeight="600" marginBottom="xs">Health Goal</Text>
        ...
        {GOAL_OPTIONS.map(({ key, label, emoji, description }) => (
          ...
          >
            ...
            >
              <Text
                variant="body"
                color={selectedGoal === key ? "primaryButtonText" : "primaryText"}
                textAlign="center"
                marginBottom="xs"
              >
                {emoji} {label}
              ...
              <Text
                variant="caption"
                color={selectedGoal === key ? "primaryButtonText" : "secondaryText"}
                textAlign="center"
                fontSize={11}
              >
                {description}
      ...
      <Box flexDirection="row" gap="sm" marginBottom="md">
        <Box flex={1}>
          <Text variant="body" fontWeight="600" marginBottom="xs">Age</Text>
          <Input
            value={physicalData.age?.toString() || ''}
            onChangeText={(text) => setPhysicalData(prev => ({ ...prev, age: parseInt(text) || undefined }))}
            placeholder="25"
        ...
        <Box flex={1}>
          <Text variant="body" fontWeight="600" marginBottom="xs">Weight (kg)</Text>
          <Input
            value={physicalData.weight?.toString() || ''}
            onChangeText={(text) => setPhysicalData(prev => ({ ...prev, weight: parseFloat(text) || undefined }))}
            placeholder="70"
            keyboardType="numeric"
            backgroundColor="surface"
            borderRadius="md"
      ...
      <Box flexDirection="row" gap="sm" marginBottom="md">
        <Box flex={1}>
          <Text variant="body" fontWeight="600" marginBottom="xs">Height (cm)</Text>
          <Input
            value={physicalData.height?.toString() || ''}
            onChangeText={(text) => setPhysicalData(prev => ({ ...prev, height: parseInt(text) || undefined }))}
            placeholder="170"
            keyboardType="numeric"
            backgroundColor="surface"
            borderRadius="md"
        ...
        <Box flex={1}>
          <Text variant="body" fontWeight="600" marginBottom="xs">Gender</Text>
          <Box flexDirection="row" gap="xs">
            {(['male', 'female', 'other'] as const).map((gender) => (
              ...
              >
                <Box
                  padding="xs"
                  backgroundColor={physicalData.gender === gender ? "primary" : "surface"}
                  borderRadius="md"
                  alignItems="center"
                >
                  <Text
                    variant="caption"
                    color={physicalData.gender === gender ? "primaryButtonText" : "primaryText"}
      ...
      <Box gap="xs" marginBottom="md">
        {(Object.keys(TDEECalculator.getActivityDescriptions()) as ('sedentary' | 'lightly_active' | 'moderately_active' | 'very_active' | 'extremely_active')[]).map((level) => (
          ...
          >
            <Box
              padding="sm"
              backgroundColor={physicalData.activityLevel === level ? "primary" : "surface"}
              borderRadius="md"
              borderWidth={1}
              borderColor={physicalData.activityLevel === level ? "primary" : "border"}
            >
              <Text
                variant="body"
                color={physicalData.activityLevel === level ? "primaryButtonText" : "primaryText"}
                textTransform="capitalize"
                marginBottom="xs"
              >
                {level.replace('_', ' ')}
              ...
              <Text
                variant="caption"
                color={physicalData.activityLevel === level ? "primaryButtonText" : "secondaryText"}
              >
      ...
      <Box flexDirection="row" gap="sm">
        ...
        <Button variant="secondary" flex={1} onPress={() => setShowCalculator(false)}>
